...
from datautils import H5ClassificationDataset

from architecture.mytransformer import ACMIL_MYMHA_EMA, ACMIL_MYMHA, sigmax
...
def get_arguments():
    ...
    parser.add_argument("--ckpt_dir", type=str, default=None, help="ckpt_dir")
    
    parser.add_argument(
        "--n_drop", type=int, default=0, help="randomly drop N tokens."
    )
    ...
    parser.add_argument(
        "--temp", type=float, default=1.0, help="learning rate"
    )
...
def main():
    ...
    # train_data, val_data, test_data = build_HDF5_feat_dataset(os.path.join(conf.data_dir, 'bracs_patch_feats_pretrain_%s.h5'%conf.pretrain), conf)
    # # train_data, val_data, test_data = build_HDF5_feat_dataset(os.path.join(conf.data_dir, 'camelyon_patch_feats_pretrain_medical_ssl.h5'), conf)

    train_data  = H5ClassificationDataset(root_dir=conf.data_dir, split='train', seed=42)
    
    val_data = H5ClassificationDataset(root_dir=conf.data_dir, split='val',   seed=42)
    test_data = H5ClassificationDataset(root_dir=conf.data_dir, split='test',   seed=42)
    ...
    model = ACMIL_MYMHA(conf, n_token=conf.n_token, n_drop=conf.n_drop)
    EMA_model = ACMIL_MYMHA_EMA(conf, n_token=conf.n_token)
    ...
    for epoch in range(train_epoch):
        train_one_epoch(model, EMA_model, criterion, train_loader, optimizer0, device, epoch, conf, args.temp)
        ...
        save_model(conf=conf, model=model, optimizer=optimizer0, epoch=epoch,
            save_path=os.path.join(ckpt_dir, 'checkpoint_{}.pth'.format(epoch)))
        
    # save_model(conf=conf, model=model, optimizer=optimizer0, epoch=epoch,
    #     save_path=os.path.join(ckpt_dir, 'checkpoint-last.pth'))
    print(args.n_token, args.n_drop ,args.temp, flush=True)
    print("Results on best epoch:")
    print(best_state, flush=True)
    wandb.finish()

def train_one_epoch(model,EMA_model, criterion, data_loader, optimizer0, device, epoch, conf, temp):
    """
    ...
    for data_it, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # Compute loss
        sub_preds, slide_preds, attn_ = model(image_patches)
        with torch.no_grad():
            EMA_attn = EMA_model(image_patches)
            EMA_attn = EMA_attn.detach()
            ema_decay = 0.999
            msd = model.state_dict()
            esd = EMA_model.state_dict()
            for k in msd.keys():
                if torch.isnan(msd[k]).any():
                    esd[k].copy_(ema_decay * esd[k] + (1. - ema_decay) * msd[k])
        ...
        attn = torch.softmax(attn_, dim=-1)
        EMA_attn= sigmax(EMA_attn/temp, dim=-1)
        # for i in range(conf.n_token):
        #     for j in range(i + 1, conf.n_token):
        #         diff_loss += torch.cosine_similarity(attn[:, i], attn[:, j], dim=-1, eps=1e-8).mean() / (
        #                     conf.n_token * (conf.n_token - 1) / 2)
        # breakpoint()
        attn = torch.softmax(attn_ + 1e-8, dim=-1)
        kdloss = -torch.nan_to_num(EMA_attn*(attn.log())).mean()
        loss = diff_loss + loss0 + loss1 + kdloss
...
def evaluate(net, criterion, data_loader, device, conf, header):
    ...
    for data in metric_logger.log_every(data_loader, 100, header):
        ...
        div_loss = 0 #torch.sum(F.softmax(attn, dim=-1) * F.log_softmax(attn, dim=-1)) / attn.shape[1]
        ...
        metric_logger.update(div_loss=0)
