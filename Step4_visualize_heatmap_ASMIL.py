...
def main():
    ...
    group_name = 'ds_%s_%s_arch_attnmil_ntoken_%s_nmp_%s_ori' % (conf.dataset, conf.pretrain, conf.n_token, conf.n_masked_patch)
    ...
    # probs = torch.softmax(attn_scores/ 0.18, dim=-1).mean(0).mean(0).cpu().numpy()
    attn_scores = attn_scores-attn_scores.mean()
    probs = torch.softmax(attn_scores/ 0.2, dim=-1)[0,0].cpu().numpy()
