...
def main():
    ...
    #     sys.exit(1)
    # breakpoint()
    net = ACMIL_MYMHA(conf, n_token=args.n_token, n_drop=args.n_drop)

    checkpoint = torch.load(ckpt_pth, weights_only=False)
    ...
    preds, feats, attn_scores = net(feat)
    # breakpoint()
    # visualize the heatmap of ACMIL
    criterion = nn.CrossEntropyLoss()
    # breakpoint()
    loss = criterion(preds.mean(0)[None,:], torch.tensor([0]).cuda())
    print(loss.item(), ", ", end='')
    output_path = os.path.join(vis_dir, slide_id+ '_' + args.ckpt + '.png')
    probs = torch.softmax(attn_scores, dim=-1).mean(0).mean(0).cpu().numpy()

    # probs = torch.softmax(attn_scores/ 0.2, dim=-1).mean(0).mean(0).cpu().numpy()
    # probs = probs-probs.min()
    # probs = probs/probs.sum()
    

    # probability_folder = os.path.join('./MYMILattention', slide_id)
    # if not os.path.exists(probability_folder):
    #     os.mkdir(probability_folder)
    # probability_file = os.path.join(probability_folder, args.ckpt.split('_')[-1]+'.pt')
    
    # torch.save(probs, probability_file)
